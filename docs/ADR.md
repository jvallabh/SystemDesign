...
## ADR-12: Preact via `compat`, with `@astrojs/preact` pinned in lockstep with Astro

**Context.** The React client runtime was 186,619 bytes against 5–10 KB per simulation chunk — the framework was ~20× the payload of the code it rendered, on every page carrying a sim. The 13 sims use only `useState`/`useEffect`/`useRef`/`useMemo` and the `ReactNode` type: no portals, Suspense, context, or third-party React libraries. **Decision.** `@astrojs/preact({ compat: true })`, aliasing `react`/`react-dom` to `preact/compat` at the Vite and TypeScript layers so sim sources keep importing from `'react'` unchanged. Total `dist/_astro` JS fell 280,288 → 112,671 bytes (−59.8%); a sim page's fetched JS fell ~85%. **Consequences.** **The `@astrojs/preact` major is load-bearing and must move in lockstep with Astro's Vite major** — `^4.1.3` depends on `vite ^6.4.1`, which dedupes against Astro 5.18's Vite 6.x into a single copy. The 5.x and 6.x lines require Vite 7 and 8; installing one against Astro 5 silently produces a *second* Vite in the tree. Note the trap: the latest published `@astrojs/preact` is 6.x, so a routine "upgrade to latest" is exactly the wrong move until Astro itself moves. Also: `preact/compat` remaps `onChange` to the native `input` event (verified across keyboard, drag, and programmatic paths), event handlers must read `e.currentTarget` rather than `e.target`, and a future third-party React dependency shipping CJS may need `vite.ssr.noExternal`.
