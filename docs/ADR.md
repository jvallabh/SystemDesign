...
## ADR-14: The Atlas Graph embeds as a `client:load` island with SSR

**Context.** The Atlas Graph (`/atlas`) is a full-page interactive concept map of all 30 topics — the site's namesake feature. Unlike the Design Studio (ADR-12), it has **no client-only inputs**: its layout is a deterministic pure function of build-time data (`buildGraph` + `computeLayout`, seeded mulberry32, no `Math.random`/`Date.now`/`performance.now`), so the server-rendered SVG is byte-identical to the client's first render. **Decision.** Embed `<AtlasMap client:load graph={graph} positions={positions} />` — `client:load` **with** SSR, not `client:only`. The deterministic layout means there is no hydration-mismatch class; SSR gives an instant paint, an indexable map, and real `<a>` node links that work with JS off. `client:load` (not `client:visible`) because the island is the sole above-the-fold content on the page. Interactivity (hover/focus neighbourhood highlight, hand-rolled pan/zoom) is added only on hydration; there is no animation loop in v1, and the single opacity transition is gated behind `prefers-reduced-motion`. A dev-only guard re-runs `computeLayout` and asserts deep-equality to protect the determinism the decision rests on. **Consequences.** Works before JS and is crawlable (30 `<a>` topic links in the static SVG); no `?d=`-style client state to hide behind a skeleton; the per-flagship island-count check generalizes to `/atlas/` (exactly one `<astro-island>`). Category colour is dynamic (per-node) so it flows through style objects (`style={{ fill: 'var(--cat-x)' }}`) per ADR-2; static SVG colour stays on the `.f-*`/`.s-*`/`.svg-label` classes.
